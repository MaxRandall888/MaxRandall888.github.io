...
import { ASTRO } from './pages/AstroDetail.jsx';
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (const st of stars) {
        ...
        ctx.globalAlpha = Math.min(1, Math.max(0.2, st.b * tw));
  ...
  return (
    <header className="sticky top-0 z-20 backdrop-blur bg-black/40 border-b border-white/10">
      <nav className="mx-auto max-w-6xl px-4 h-14 flex items-center justify-between">
            <Link to="/" onClick={handleNav('home')} className="font-bold text-slate-100 tracking-tight">
              Max Randall
            </Link>
...
function AstroGrid() {
    // Pick 6 featured IDs to show on the landing page
    const featured = [
        "andromeda",
        "creascentw",
        "m101-2",
